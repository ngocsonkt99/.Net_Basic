using System;
using System.Collections.Generic;
using System.Data;
using System.Data.Entity;
using System.Linq;
using System.Net;
using System.Web;
using System.Web.Mvc;
using DataAccess;
using Model;
using WEB.XuLy;

namespace WEB.Controllers
{
    public class LoaiXesController : Controller
    {
        private XeContext db = new XeContext();

        // GET: LoaiXes
        public ActionResult Index()
        {
            return View(db.LoaiXees.ToList());
        }

        XuLyLoaiXe xl = new XuLyLoaiXe();

        public PartialViewResult DanhSachLoaiXe()
        {
            return PartialView(xl.getDs());
        }

        public ActionResult LayXeTheoLoai(int id)
        {
            List<Xe> l = xl.getXeById(id);

            return View(l);
        }

        // GET: LoaiXes/Details/5
        public ActionResult Details(int? id)
        {
            if (id == null)
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
            }
            LoaiXe loaiXe = db.LoaiXees.Find(id);
            if (loaiXe == null)
            {
                return HttpNotFound();
            }
            return View(loaiXe);
        }

        // GET: LoaiXes/Create
        public ActionResult Create()
        {
            return View();
        }

        // POST: LoaiXes/Create
        // To protect from overposting attacks, please enable the specific properties you want to bind to, for 
        // more details see https://go.microsoft.com/fwlink/?LinkId=317598.
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Create([Bind(Include = "Id,TenLoai")] LoaiXe loaiXe)
        {
            if (ModelState.IsValid)
            {
                db.LoaiXees.Add(loaiXe);
                db.SaveChanges();
                return RedirectToAction("Index");
            }

            return View(loaiXe);
        }

        // GET: LoaiXes/Edit/5
        public ActionResult Edit(int? id)
        {
            if (id == null)
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
            }
            LoaiXe loaiXe = db.LoaiXees.Find(id);
            if (loaiXe == null)
            {
                return HttpNotFound();
            }
            return View(loaiXe);
        }

        // POST: LoaiXes/Edit/5
        // To protect from overposting attacks, please enable the specific properties you want to bind to, for 
        // more details see https://go.microsoft.com/fwlink/?LinkId=317598.
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "Id,TenLoai")] LoaiXe loaiXe)
        {
            if (ModelState.IsValid)
            {
                db.Entry(loaiXe).State = EntityState.Modified;
                db.SaveChanges();
                return RedirectToAction("Index");
            }
            return View(loaiXe);
        }

        // GET: LoaiXes/Delete/5
        public ActionResult Delete(int? id)
        {
            if (id == null)
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
            }
            LoaiXe loaiXe = db.LoaiXees.Find(id);
            if (loaiXe == null)
            {
                return HttpNotFound();
            }
            return View(loaiXe);
        }

        // POST: LoaiXes/Delete/5
        [HttpPost, ActionName("Delete")]
        [ValidateAntiForgeryToken]
        public ActionResult DeleteConfirmed(int id)
        {
            LoaiXe loaiXe = db.LoaiXees.Find(id);
            db.LoaiXees.Remove(loaiXe);
            db.SaveChanges();
            return RedirectToAction("Index");
        }

        protected override void Dispose(bool disposing)
        {
            if (disposing)
            {
                db.Dispose();
            }
            base.Dispose(disposing);
        }
    }
}
